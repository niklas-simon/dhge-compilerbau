...
static void check_arg_cnt(const treenode_t *t, int expected)
{
    for (int actual = 0;; ++actual)
    {
        if ((actual == MAX_ARGS) || (t->son[actual] == NULL))
        {
            if (actual != expected)
            {
                code_error(&(t->pos), "Die Anzahl der Werte im Aufruf (%d) "
...
static void pop_mark(void)
{
    if (top_mark > first_mark)
        --top_mark; // kein pop bei leerem Stack
}
...
void print(const treenode_t *tree, const int indent)
{
    ...
    for (int i = 0; i < MAX_ARGS; i++)
    {
        if (tree->son[i])
        {
            print(tree->son[i], indent + 4);
        }
    }
    if (tree->next)
    {
        print(tree->next, indent);
    }
}
